<?php $page = 'part-requirements.edit'; ?>
    ...
    <div class="page-wrapper">
        <div class="content pb-0">
            ...
            <div class="d-flex align-items-center justify-content-between gap-2 mb-4 flex-wrap">
                <div>
                    <h4 class="mb-1">Edit Part Requirement</h4>
            ...
            <div class="card border-0 rounded-0">
                <div class="card-body">
                    <form action="{{ route('part-requirements.update', $requirement->id) }}" method="POST" id="edit-form">
                        @csrf
                        @method('PUT')
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="equipment_id" class="select2 form-select" required>
                                    ...
                                    @foreach($equipments as $equipment)
                                        <option value="{{ $equipment->id }}" {{ $requirement->equipment_id == $equipment->id ? 'selected' : '' }} data-service-period="{{ $equipment->periodicServiceType ? $equipment->periodicServiceType->name : '-' }}">{{ $equipment->code }} - {{ $equipment->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Service Period</label>
                                <input type="text" id="service_period_display" class="form-control" readonly value="{{ ($requirement->equipment && $requirement->equipment->periodicServiceType) ? $requirement->equipment->periodicServiceType->name : '-' }}">
                            </div>
                            <div class="col-md-6 mb-3">
                            ...
                            <div class="col-md-12 mb-3">
                                ...
                                <textarea name="description" class="form-control" rows="2">{{ $requirement->description }}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="table-responsive mb-3">
                            <table class="table table-bordered" id="parts-table">
                                ...
                                <tbody>
                                    @foreach($requirement->details as $index => $detail)
                                    <tr>
                                        <td>
                                            <select name="parts[{{ $index }}][part_id]" class="select2 form-select" required>
                                                <option value="">Select Part</option>
                                                @foreach($parts as $part)
                                                    <option value="{{ $part->id }}" {{ $detail->part_id == $part->id ? 'selected' : '' }}>{{ $part->name }}</option>
                                                @endforeach
                                            </select>
                                        </td>
                                        <td>
                        ...
                        <div class="d-flex align-items-center justify-content-end gap-2">
                            ...
                            <button type="submit" class="btn btn-primary" id="btn-submit">Update Requirement</button>
                        </div>
                    </form>
                </div>
            </div>
...
$(document).ready(function() {
    ...
    $('select[name="equipment_id"]').on('change', function() {
        ...
        const servicePeriod = selectedOption.data('service-period') || '-';
        $('#service_period_display').val(servicePeriod);
    });

    let rowIdx = {{ $requirement->details->count() }};
