$(document).ready(function () {
            ...
            success: function (res) {
                if (res.success) {
                    ...
                    if (data.work_date) {
                        ...
                        // Adjust for timezone offset if necessary, or strictly use the string from DB if available as YYYY-MM-DD
                        // Laravels default JSON serialization of date is ISO 8601. 
                        // To set input[type=date], we need YYYY-MM-DD
    ...
    $('#activity-form').on('submit', function (e) {
        ...
        btn.prop('disabled', true).html('<span class="spinner-border spinner-border-sm me-1"></span> Logging...');
        ...
        $.ajax({
            ...
            error: function (xhr) {
                Swal.fire('Error', xhr.responseJSON?.message || 'Error logging activity.', 'error');
            },
            complete: function () {
                                ...
                                <div class="d-flex gap-3 mb-3 border-bottom pb-2">
                                    ...
                                    <div class="flex-grow-1">
                                        <div class="d-flex justify-content-between align-items-center mb-1">
                                            <h6 class="mb-0 fs-13">${act.mechanic ? act.mechanic.name : 'Unknown'}</h6>
                                            <small class="text-muted">${formatDate(act.created_at)}</small>
                                        </div>
                                        <p class="text-muted mb-1 small">${act.description}</p>
                                        <span class="badge bg-light text-dark border">${act.status}</span>
                                    </div>
                                </div>
                            `;
                        });
        ...
        Swal.fire({
            ...
            preConfirm: (qty) => {
                return $.ajax({
                    ...
                    method: 'POST',
                    data: { qty_issued: qty, _token: $('meta[name="csrf-token"]').attr('content') }
                }).then(response => {
                    return response;
                }).catch(error => {
            ...
            if (result.isConfirmed) {
                ...
                // Refresh detail view
                const openedId = $('#view-wo-title').data('id'); // We need to store ID somewhere
    ...
    $(document).on('click', '.btn-sp-return', function () {
        ...
        Swal.fire({
            ...
            input: 'number',
            inputLabel: `Quantity Returned (Max: ${max})`,
            inputAttributes: { max: max, step: 0.01 },
            showCancelButton: true,
            ...
            preConfirm: (qty) => {
                return $.ajax({
                    ...
                    method: 'POST',
                    data: { qty_returned: qty, _token: $('meta[name="csrf-token"]').attr('content') }
                }).then(response => {
                    return response;
                }).catch(error => {
                    Swal.showValidationMessage(`Request failed: ${error.responseJSON.message}`);
